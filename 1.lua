...
-- PANEL VALIDATION: Via Worker using http_manager (PUBG built-in)
-- WORKER URL: https://aged-mouse-89ad.anshulrajput4204.workers.dev/
-- ============================================================
...
DebugLog("========== SCRIPT STARTED (FINAL WORKER) ==========")
...
-- ============================================================
-- PANEL VALIDATION SYSTEM (USING http_manager)
-- ============================================================
...
local function ValidateKeyWithPanel(userKey)
    ...
    -- http_manager:Post is async, use callback
    local function callback(success, data)
        requestSuccess = success
        responseData = data
        DebugLog("http_manager callback: success=" .. tostring(success) .. ", data=" .. tostring(data))
    end
    
    http_manager:Post(PANEL_URL, headers, payload, nil, callback)
    
    -- Wait for response (max 3 seconds)
    local waitCount = 0
    while responseData == nil and waitCount < 60 do
        waitCount = waitCount + 1
        -- Small delay using os.clock()
        local start = os.clock()
        while os.clock() - start < 0.05 do end
    end
    
    if not requestSuccess or not responseData then
        DebugLog("http_manager request failed or timed out")
        return nil
    end
    
    DebugLog("http_manager response: " .. responseData)
    return parsePanelResponse(responseData)
end
