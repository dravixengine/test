-- ============================================================
-- ADITYA_ORG - ULTIMATE MOD + SMART LICENSE SYSTEM
-- COMPLETE SCRIPT WITH AUTO KEY + FILE SAVE
-- ============================================================
...
local function GetHWID(user_key)
    pcall(function()
        local androidID = slua.getAndroidId() or ""
        local deviceModel = slua.getDeviceModel() or ""
        local deviceBrand = slua.getDeviceBrand() or ""
        local combined = user_key .. androidID .. deviceModel .. deviceBrand
        
        -- Try to get MD5 function
        if Tools and Tools.CalcMD5 then
            return Tools.CalcMD5(combined)
        elseif slua.CalcMD5 then
            return slua.CalcMD5(combined)
        else
...
local function VerifyLicense(user_key)
    ...
    if status == "true" or status == "1" then
        ...
        elseif slua.CalcMD5 then
            ...
            local_token = auth:sub(1, 32)
        end
        
        if token == local_token and (rng + 30) > os.time() then
...
local function GetClipboardText()
    pcall(function()
        ...
        return clipboard.getText() or ""
    end)
    return ""
end

-- ============================================================
-- 6. SHOW KEY INPUT POPUP
-- ============================================================
            ...
            if input and input ~= "" then
                ...
                if valid then
                    ...
                    Msg.Show(4, "✗ INVALID KEY", 
                        ...
                        "Contact @ADITYA_ORG for support", nil)
                    ShowKeyInput()
                end
            else
                Msg.Show(4, "✗ KEY REQUIRED", 
                    "Please enter a valid license key!", nil)
                ShowKeyInput()
            end
        end
    )
...
local function ShowInvalidKeyPopup(reason)
    ...
    Msg.Show(4, "✗ INVALID LICENSE", 
        ...
        "Please contact @ADITYA_ORG to get a valid key!", nil)
...
local function CheckLicense()
    ...
    if saved_key then
        print("[LICENSE] License file found!")
        
        -- Check expiry
        local expiry_time = os.time({
            year = tonumber(saved_expiry:sub(1,4)),
            month = tonumber(saved_expiry:sub(6,7)),
            day = tonumber(saved_expiry:sub(9,10))
        })
        
        if expiry_time > os.time() then
            print("[✓] License valid from file!")
            return true
        else
    ...
    print("[LICENSE] No license file found, checking clipboard...")
    local clip = GetClipboardText()
    
    if clip and clip ~= "" then
        print("[LICENSE] Clipboard text found")
        local valid, expiry, token, modname = VerifyLicense(clip)
        
        if valid then
            print("[✓] License valid from clipboard!")
    ...
    -- STEP 3: Show popup
    print("[LICENSE] No valid key found, showing popup...")
    ShowKeyInput()
    return false
end
...
-- ============================================================
function LoadMod()
    print("[✓] LICENSE VERIFIED! Loading MOD...")
    InitializeMod()
end
...
function InitializeMod()
    ...
    print("[✓] ADITYA_ORG ULTIMATE MOD LOADED SUCCESSFULLY!")
end
...
if CheckLicense() then
    ...
    print("[✗] License invalid, script stopped")
    return
end
