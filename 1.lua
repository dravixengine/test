...
-- ============================================================
-- CHAMS COLOR CONFIG (for ESP name colors)
-- ============================================================
...
local COLOR_7 = {
    ...
    [2] = {R=100, G=100, B=1.0, A=100},   -- White
    [3] = {R=100, G=100, B=0.0, A=100},   -- Yellow
    [4] = {R=0.0, G=100, B=0.0, A=100},   -- Green
    [5] = {R=0.0, G=100, B=100, A=100},   -- Cyan
    [6] = {R=0.0, G=0.0, B=100, A=100},   -- Blue
    [7] = {R=100, G=0.0, B=100, A=100},   -- Purple
}
...
local function ChamsApplyToMesh(mesh)
    if not mesh or not slua.isValid(mesh) then return end

    local visIdx = _G.ESPConfig.WallhackVisibleColor or 4
    local occIdx = _G.ESPConfig.WallhackInvisibleColor or 3
...
-- VEHICLE ESP
-- ============================================================
local VEHICLE_ESP_TIMER = nil
local _VehicleCacheTime = 0
local _VehicleCache = {}

function GetVehicleDisplayName(veh)
    if not slua.isValid(veh) then return "Vehicle" end
    local name = veh.DisplayName
    if name and name ~= "" then return name end

    local avatarComp = veh.VehicleAvatar
    if slua.isValid(avatarComp) then
        local defaultID = avatarComp:GetDefaultAvatarID() or 0
        if defaultID and defaultID > 0 then
            local baseID = math.floor(defaultID / 1000) * 1000
            local nameMap = {
                [1901000] = "Motorcycle",
            ...
            return nameMap[baseID] or "Vehicle"
        end
    end
    return "Vehicle"
end
        ...
        if slua.isValid(vehicle) then
            ...
            if vPos then
                ...
                if distSq < MAX_DIST_SQ then
                    ...
                    local label = vehicleName .. " [" .. math.floor(dist/100) .. "m]"
...
local function StartVehicleESP()
    if VEHICLE_ESP_TIMER then
        pcall(function()
            if _G.Game then _G.Game:RemoveGameTimer(VEHICLE_ESP_TIMER) end
        end)
        VEHICLE_ESP_TIMER = nil
    end
    local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
    if slua.isValid(pc) and pc.AddGameTimer then
        VEHICLE_ESP_TIMER = pc:AddGameTimer(0.5, true, function()
            pcall(VehicleESPLoop)
        end)
        print("[VEHICLE ESP] ✅ Started")
        return true
    end
    return false
end

local veRetryCount = 0
local function RetryVehicleESP()
    if veRetryCount >= 30 then return end
    veRetryCount = veRetryCount + 1
    if StartVehicleESP() then
        print("[VEHICLE ESP] ✅ Ready!")
    else
        if _G.Game and _G.Game.AddGameTimer then
            _G.Game:AddGameTimer(1.0, false, RetryVehicleESP)
        end
    end
end
...
-- ============================================================
-- ==================== SKINS MODULE ===========================
-- (Full advanced skin system from 1.lua)
-- ============================================================
